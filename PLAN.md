...
| **1 · Cohort & labels** | counts reconcile; zero patient overlap across splits | `[~]` 1.2 (labels) remaining — blocked on credentialing |
...
`subject_id`/`hadm_id`. The ED module alone supports only label 4.

**Three things now block on the same join**, which is worth seeing as one dependency
rather than three: the outcome labels, `patients.anchor_age` for an adult-only cohort
(§3.2), and `patients.anchor_year_group` for a genuine temporal split (§4). Each one
currently *refuses* rather than silently degrading. This is the concrete
...
- [x] **1.1** Cohort definition + exclusion reporting **per outcome class** (§3.2). Every
      filter records *which* stays it removed, not just how many, so the audit can run once
      labels exist rather than having to be planned in advance. `stratify()` reports a
      **disparity** column — the widest gap in removal rate between outcome classes — and
      **refuses to run when the numerator and denominator count different units.**
      LWBS/eloped/AMA are retained and flagged as censored, never excluded. *(2026-08-26)*
...
- [x] **1.4** Patient-grouped splits + the zero-overlap assertion, tested by planting a
      ...
      *(2026-08-26)*
- [x] **1.5** Cohort characterisation → `reports/cohort.md`, generated by
      `scripts/cohort_report.py` with its full config embedded. *(2026-08-26)*
